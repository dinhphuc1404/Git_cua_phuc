...
@MultipartConfig
@WebServlet(name = "QuestionServlet1", urlPatterns = "/teacher/questions")
public class QuestionServletTeacher extends HttpServlet {
...
//        request.getRequestDispatcher("/teacher/question-bank.jsp").forward(request,response);
//    }
private void searchQuestion(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    String description = request.getParameter("description");
    if(description == null || description.trim().isEmpty()) { // Kiểm tra mô tả câu hỏi có giá trị rỗng hay không
        response.setStatus(HttpServletResponse.SC_OK); // Đặt trạng thái yêu cầu HTTP là OK
        response.sendRedirect("question-bank.jsp"); // Chuyển hướng yêu cầu HTTP trở lại trang question-bank.jsp
    } else {
        List<Question> questions = questionService.findAllByDescription(description);
        request.setAttribute("listQuestion", questions);
        List<Subject> listSubject = subjectService.selectAllSubject();
...
//        response.sendRedirect("/questions");
    ...
    private void questionList(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        RequestDispatcher dispatcher = request.getRequestDispatcher("/teacher/question-bank.jsp");
...
//         chổ ni ae tự thêm đường link foder uploads của dự án vào
        String realPath2 = "E:\\web_trac_nghiem\\KhoaLuanDTU\\fileQuestion";
